...
import { CheckCircle, XCircle, ArrowLeft, Loader2, MessageCircle, ExternalLink, Phone, Mail, User } from 'lucide-react';
...
export default function AdminBookingDetail({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: bData, error } = await supabase
        .from('bookings')
        .select(`*, service:services(*), profile:profiles(full_name, email, phone_number)`)
        .eq('id', id)
        .single();
  ...
  const handleVerify = async (paymentId: string, status: 'verified' | 'rejected') => {
    ...
    try {
        ...
        // Pastikan tipe data kolom 'status' di tabel 'payments' menerima 'verified'/'rejected'
        // Jika di DB enum-nya 'success'/'failed', ganti string di bawah ini.
        const { error: paymentError } = await supabase
          .from('payments')
          .update({ 
              status: status, // Menggunakan status langsung ('verified' atau 'rejected')
        ...
        if (status === 'verified') {
            await supabase.from('bookings').update({ 
                status: 'confirmed',
                payment_status: 'paid',
                total_paid: booking.total_price // Asumsi lunas
            }).eq('id', id);

            toast.success("Pembayaran Diterima!");
            
            // Kirim WA Notifikasi
            const message = `Halo ${booking.customer_name}, pembayaran Anda untuk booking ${booking.service?.name} telah kami TERIMA LUNAS. Silakan cek e-tiket di dashboard website. Terima kasih!`;
            ...
            const message = `Halo ${booking.customer_name}, bukti pembayaran Anda untuk ${booking.service?.name} kami TOLAK. Mohon upload ulang bukti yang benar.`;
            window.open(`https://wa.me/${booking.customer_phone}?text=${encodeURIComponent(message)}`, '_blank');
        }
        ...
        // PERBAIKAN: Handling unknown error
        let message = "Gagal memproses";
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Verifikasi Pembayaran</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-6 pt-6">
              <div>
                  <p className="text-xs text-slate-500 font-bold uppercase mb-1">Layanan</p>
                  <p className="text-xl font-bold text-blue-600">{booking.service?.name}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <p className="text-xs text-slate-500 mb-1">Check-in</p>
                      <p className="font-medium">{new Date(booking.start_time).toLocaleDateString('id-ID', {dateStyle: 'medium'})}</p>
                      <p className="text-xs text-slate-600">{new Date(booking.start_time).toLocaleTimeString('id-ID', {timeStyle: 'short'})}</p>
                  </div>
                  <div>
                      <p className="text-xs text-slate-500 mb-1">Check-out</p>
                      <p className="font-medium">{new Date(booking.end_time).toLocaleDateString('id-ID', {dateStyle: 'medium'})}</p>
                      <p className="text-xs text-slate-600">{new Date(booking.end_time).toLocaleTimeString('id-ID', {timeStyle: 'short'})}</p>
                  </div>
              </div>
              ...
              <div className="space-y-2">
                  <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-900">{formatRupiah(booking.total_price)}</span>
                  </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6 pt-6">
              ...
              <Button 
                ...
                onClick={() => window.open(`https://wa.me/${booking.customer_phone}`, '_blank')}
              >
      ...
      <h2 className="text-xl font-bold text-slate-900">Riwayat Bukti Transfer</h2>
      ...
      <div className="space-y-4">
        {payments.map((payment) => {
           ...
           return (
            <Card key={payment.id} className={`overflow-hidden transition-all ${payment.status === 'pending' ? 'border-blue-300 shadow-md ring-1 ring-blue-100' : 'opacity-80'}`}>
                    ...
                    <Dialog>
                        <DialogTrigger asChild>
                            <div className="w-full md:w-64 h-48 bg-slate-100 relative cursor-zoom-in group shrink-0">
                                {imgUrl ? (
                                    <>
                                        <Image src={imgUrl} alt="Bukti" fill className="object-cover" />
                        ...
                        <DialogContent className="max-w-4xl p-0 bg-transparent border-none shadow-none">
                            <div className="relative w-full h-[80vh]">
                                {imgUrl && <Image src={imgUrl} alt="Bukti Full" fill className="object-contain" />}
                            </div>
                        </DialogContent>
                    </Dialog>
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-between">
                        <div>
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <div className="text-2xl font-bold text-slate-900">{formatRupiah(payment.amount)}</div>
                                    <div className="text-sm text-slate-500 mt-1">Via {payment.payment_type}</div>
                                </div>
                                <Badge 
                            ...
                            <p className="text-xs text-slate-400">
                                Diupload pada: {new Date(payment.created_at).toLocaleString()}
                            </p>
                        </div>
                        ...
                        {isVerified && (
                            <div className="mt-4 text-sm text-green-600 font-medium flex items-center gap-2">
                                <CheckCircle className="w-4 h-4"/> Pembayaran ini telah diverifikasi valid.
                            </div>
                        )}
                    </div>
                </CardContent>
